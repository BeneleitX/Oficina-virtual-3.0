...
/*         foreach( $this->data["socio"]->data->verificacion as $j => $k){
    ...
    public function cancela_ine( string $tipo ){
        ...
        if( file_exists( "data/{$this->data[ "socio" ]->id}/ine/".$this->data["socio"]->data->credencial->{$tipo} ) ){
            @rename( 
    ...
    public function add_beneficiario(){
        ...
        $nuevo = [
            "nombre" => preg_replace('/[^A-Za-z0-9\-]/', ' ', $this->request->getPost( "beneficiario_nuevo" ) ),
    ...
    public function guarda_clabe(){
        ...
        if( $db->query( "select count(*) as existe from t_usuarios where id != {$socio->id} and estatus_codigo = '201-ACTIVO' and data->>'$.clabe' = '{$clabe}' ")->getRow()->existe ){
            ...
            return redirect()
                ->back()
                ->with( "errors", [ "clabe" => "CLABE ya utilizada por otro socio" ] )
                ->withInput();           
        }
    ...
    public function update_estatus( $s ){
        ...
        echo $sql = "select f_update_PTS( {$s}, codigo, '{$mes_anterior}' ) FROM t_modelos WHERE estatus_codigo = '201-ACTIVO'";
        $db->query( $sql );  
    ...
    public function check_csf(){
        ...
        $json = $this->data["socio"]->data;
        $json->sat->estatus = $this->request->getPost( "check" ) == "true" ? 0 : ( ( $json->sat->csf == null || $json->sat->csf == "null" ) ? 1 : 2 );
        $this->data["socio"]->data = $json; 
