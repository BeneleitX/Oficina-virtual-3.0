...
-- Dumping structure for procedure app_v22.p_avance_corte
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_cobra_estrellas
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_genera_pagos
DELIMITER //
		...
		pagos: BEGIN
		    ...
		    lop_pagos : LOOP
	            ...
	            SET d_data = JSON_OBJECT(
	                "retencion", impuestos,
	                "menor", d_menor,
	                "verificado", JSON_EXTRACT( f_es_verificado( d_usuario ), '$.estatus' ) AND IF( d_modelo = '50-INVERSION', d_wallet is not null and length( d_wallet ) = 34, d_clabe is not null and length( d_clabe ) = 18 ),
	                "factura", null,
...
-- Dumping structure for procedure app_v22.p_get_inversiones
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_get_paquetes
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_update_niveles
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_update_padre
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_update_primercompra
DELIMITER //
...
-- Dumping structure for procedure app_v22.p_update_rango
DELIMITER //
...
-- Dumping structure for function app_v22.f_calcula_estrellas
DELIMITER //
CREATE FUNCTION `f_calcula_estrellas`(`input_pedido` INT) RETURNS decimal(5,2)
    DETERMINISTIC
BEGIN
...
-- Dumping structure for function app_v22.f_calcula_isr
DELIMITER //
...
-- Dumping structure for function app_v22.f_checks_rango
DELIMITER //
...
-- Dumping structure for function app_v22.f_compresion_de_red
DELIMITER //
...
-- Dumping structure for function app_v22.f_compulsa_valores
DELIMITER //
...
-- Dumping structure for function app_v22.f_estrellas_en_pedido
DELIMITER //
...
-- Dumping structure for function app_v22.f_es_verificado
DELIMITER //
...
-- Dumping structure for function app_v22.f_fecha_primercompra
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_avatar
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_calificacion
DELIMITER //
...
BEGIN
	...
	IF JSON_CONTAINS( roles, '"42-PERMANENTE"', '$') OR JSON_CONTAINS( roles, '"41-PERMANENTE-U"', '$') THEN
		CASE
	...
	if primercompra IS not NULL then
		...
		CASE
		    ...
		    WHEN modelo = '20-TELEFONIA' THEN 	
				...
				SELECT pr.data->'$.puntos.\"310-TELEFONIA\"'
				into respuesta
				FROM t_pedidos pe
...
-- Dumping structure for function app_v22.f_get_downline
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_estatus
DELIMITER //
...
*/
    ...
    */
	...
	SELECT 
		u.rol_codigos, 
		u.historial,
		IFNULL( IF( u.data->>'$.saldo."50-INVERSION".cantidad' = '', 0, u.data->>'$.saldo."50-INVERSION".cantidad' ), 0 ),
 	...
 	INTO _roles, _historial, _directos_inv, _verificacion, _actuales, _registro
 	FROM t_usuarios u
 	WHERE u.id = _usuario
	group by u.id 
	;
	...
	WHILE _k < JSON_LENGTH( _modelos ) DO
		...
		final: LOOP
            ...
            WHEN JSON_CONTAINS( _roles, '"41-PERMANENTE-U"', '$') THEN 
...
-- return json_array(  JSON_UNQUOTE( JSON_EXTRACT( _actuales, CONCAT( '$."', _k, '"' ) ) ), _k );
            ...
            when _modelo = '50-INVERSION' then      
                ...
                ELSE
                	IF _directos_inv > 0 then
            	        SET _fechabaja   = DATE_FORMAT( _registro + INTERVAL 1800 DAY, '%Y-%m-%d' );
            	    end if;
	...
	IF curdate() > DATE_FORMAT( _registro + INTERVAL 90 DAY, '%Y-%m-%d' ) THEN
		...
		WHILE _k < JSON_LENGTH( _modelos ) DO
	            ...
	            IF SUBSTRING( JSON_UNQUOTE( JSON_EXTRACT( _estatuses, CONCAT( '$."', _modelo, '"' ) ) ), 1, 3 ) > IF( _modelo = '50-INVERSION', 200, 300 ) THEN
	                SET _estatus = false;
	            END IF;
    ...
    WHILE _k < JSON_LENGTH( _modelos ) DO
            ...
            IF SUBSTRING( JSON_UNQUOTE( JSON_EXTRACT( _estatuses, CONCAT( '$."', _modelo, '"' ) ) ), 1, 3 ) < 200 THEN
...
-- Dumping structure for function app_v22.f_get_factor_promos
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_ingresos
DELIMITER //
CREATE FUNCTION `f_get_ingresos`(`usuario` MEDIUMINT, `mes` VARCHAR(6), `modelo` VARCHAR(20)) RETURNS decimal(10,2)
    DETERMINISTIC
BEGIN
...
-- Dumping structure for function app_v22.f_get_inventario
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_niveles
DELIMITER //
CREATE FUNCTION `f_get_niveles`(`i_usuario` INT, `i_modelo` VARCHAR(20), `mes` VARCHAR(6)) RETURNS json
    DETERMINISTIC
BEGIN
...
-- Dumping structure for function app_v22.f_get_padre
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_semilla
DELIMITER //
...
-- Dumping structure for function app_v22.f_get_upline
DELIMITER //
...
-- Dumping structure for function app_v22.f_rasura_comision
DELIMITER //
CREATE FUNCTION `f_rasura_comision`(`input_pedido` MEDIUMINT, `calificacion` INT) RETURNS decimal(10,2)
    DETERMINISTIC
BEGIN
...
-- Dumping structure for function app_v22.f_rasura_movil
DELIMITER //
CREATE FUNCTION `f_rasura_movil`(`input_nivel` INT, `input_comision` DECIMAL(8,2), `input_calificacion` INT, `input_producto` VARCHAR(10), `input_esquema` VARCHAR(20)) RETURNS decimal(8,2)
    DETERMINISTIC
BEGIN
	-- declaración de variables
...
-- Dumping structure for function app_v22.f_reparte_comisiones
DELIMITER //
							    	...
							    	if '116-ANIVERSARIO' = v_esquema COLLATE utf8mb4_0900_ai_ci then
										...
										WHILE q < JSON_LENGTH( prods ) DO
											...
											IF (
												...
												AND ll_prods not IN ('230-BNOX', '170-BMEL', '310-FRSH', '206-CITR') 
											) THEN
...
-- Dumping structure for function app_v22.f_reset_padre
DELIMITER //
...
-- Dumping structure for function app_v22.f_update_nivel
DELIMITER //
CREATE FUNCTION `f_update_nivel`(`socio` INT, `modelo` VARCHAR(20), `mes` INT) RETURNS json
BEGIN
...
-- Dumping structure for function app_v22.f_update_PTS
DELIMITER //
